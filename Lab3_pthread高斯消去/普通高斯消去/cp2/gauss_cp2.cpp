...
void *thread_func_row(void *param) { //行划分
...
void *thread_func_col(void *param) { //列划分
    long long t_id = (long long)param;
    for (int k = 0; k < n; k++) {
        if (t_id == 0) {
            float ele = A[k][k];
...
void static_gauss_sem_col() {
    ...
    pthread_t *handles = new pthread_t[thread_count];
    for (int i = 0; i < thread_count; i++)
        pthread_create(handles + i, NULL, thread_func_col, (void *)(long long)i);
    for (int i = 0; i < thread_count; i++)
        pthread_join(handles[i], NULL);
    delete[] handles;
    for (int i = 0; i < thread_count - 1; i++) {
...
int main(int argc, char *argv[])
{
    ...
    paths[2] = "static_gauss_sem_col";
    void (*p[3])();
    p[0] = &common_gauss;
    p[1] = &static_gauss_sem_row;
    p[2] = &static_gauss_sem_col;
 	n = MaxN;
