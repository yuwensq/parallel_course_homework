...
void optimization_time(int n) {
    for (int i = 0; i < n; i++)
        sum[i] = 0.0;
    for (int j = 0; j < n; j++) {
        for (int i = 0; i < n - 1; i += 2) {
            sum[i] += arr[j][i] * vect[j];
            sum[i + 1] += arr[j][i + 1] * vect[j];
        }
        if (n % 2)
            sum[n - 1] += arr[j][n - 1] * vect[j];
    }
}
...
int main()
{
    ofstream output_result("optimize2way_data.txt");
    ofstream output_drp("draw_picturep2way.txt");
    int scale = 50;//测定固定规模时修改此处及循环条件即可
    while (scale <= N) {
		init(scale);
    	test(optimization_time, scale, &output_result, &output_drp);
		release_mem(scale);
        scale += 50;
    }
    output_result.close();
