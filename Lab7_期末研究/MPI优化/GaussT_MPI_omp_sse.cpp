...
#include"mpi.h"
using namespace std;
const int col = 130, elinenum = 8, num_thread = 4, mpisize = 8;//列数、被消元行数
bool parallel = 1;
int isupgrade;
int tmp = 0;
...
public:
	...
	void doxor(bitmatrix b) {  //两行做异或操作，由于结果留在本实例中，只有被消元行能执行这一操作,且异或操作后要更新首项
		__m128 v0, v1;
		int p = 0;
		for (p; p <= bytenum - 4; p += 4) {
			v0 = _mm_load_ps((float*)(&mybyte[p]));
			v1 = _mm_load_ps((float*)(&(b.mybyte[p])));
			v0 = _mm_xor_ps(v0, v1);
			_mm_store_ps((float*)(&mybyte[p]), v0);
		}
		for (p; p < bytenum; p++)
			mybyte[p] ^= b.mybyte[p];
...
void readdata() {
	ifstream ifs;
	ifs.open("/home/yuwensq/test/1.txt");  //消元子
	string temp;
	...
	ifs.close();
	ifstream ifs2;
	ifs2.open("/home/yuwensq/test/2.txt");     //被消元行,读入方式与消元子不同
	int trow = 0;
